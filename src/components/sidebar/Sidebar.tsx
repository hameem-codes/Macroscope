...
export default function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`
          fixed top-0 left-0 h-full w-64 bg-white border-r-2 border-foreground z-50 flex flex-col justify-between
          transform transition-transform duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)]
          lg:translate-x-0 lg:static lg:z-auto
          ${isOpen ? "translate-x-0" : "-translate-x-full"}
        `}
      >
        <div className="flex-1 overflow-y-auto mb-[76px] scrollbar-thin">
          {/* Brand */}
        ...
        {/* Data status */}
        <div className="absolute bottom-0 left-0 right-0 p-4 border-t-2 border-foreground bg-white">
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span className="w-2 h-2 rounded-full bg-green-500 animate-pulse" />
            <span>Live World Bank Data</span>
          </div>
          <p className="text-[10px] text-muted-foreground mt-1">
            Active indicators across 8 categories
          </p>
        </div>
      </aside>
    </>
