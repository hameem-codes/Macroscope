...
export default function GlobalRankingPage() {
  ...
  const allCountryData = countries.map((country) => ({
    country,
    data: getCountryHealthData(country.id),
    indicators: getIndicatorsForCountry(country.id),
  }));

  // Sort
  allCountryData.sort((a, b) =>
    sortBy === "score-desc"
      ? b.data.overallScore - a.data.overallScore
    ...
    <Shell>
      ...
      <section className="relative mb-8">
        ...
        <p className="text-sm text-muted-foreground font-body">
          Comparative ranking of {countries.length} economies based on simulated health scores.
        </p>
                ...
                <tr key={cat.slug} className="border-b border-border last:border-0">
                  ...
                  {allCountryData.slice(0, 5).map((item) => {
                    ...
                    const allScores = allCountryData.slice(0, 5).map((d) => {
                      const cs = d.data.categoryScores.find((c) => c.categorySlug === cat.slug);
                      return cs?.score || 0;
                    });
      ...
      <section className="mt-8 pt-6 border-t-2 border-border">
        <div className="flex items-center gap-2 text-xs text-muted-foreground font-body">
          ...
          <span>· All scores are simulated for demonstration purposes.</span>
        </div>
      </section>
    </Shell>
  );
